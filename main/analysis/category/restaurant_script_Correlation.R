...
# dbquery_tbl$price_range zeiche zählen
...
library(cluster)
?agnes
fit.agn <- agnes(dbquery_tbl, metric="manhatten", stand=TRUE) # Vector to large

library(vegan)
fit.clu <- hclust(dist(dbquery), method="single")  # Vector to large
